...
class Decoder extends Module{
    val io = IO(new Bundle{
        ...
        val ctrl_WBSel = Output(UInt(2.W))      //for write back source
    })
    ...
    io.ctrl_ASel := Mux(io.opcode === BRANCH || io.opcode === LUI || io.opcode === AUIPC || io.opcode === JAL, true.B, false.B)
    io.ctrl_BSel := Mux(io.opcode === OP, false.B, true.B)
    io.ctrl_Br := Mux(io.opcode === BRANCH, true.B, false.B)
    io.ctrl_Jmp := Mux(io.opcode === JALR || io.opcode === JAL, true.B, false.B)
    io.ctrl_Lui := Mux(io.opcode === LUI, true.B, false.B)
    io.ctrl_MemRW := Mux(io.opcode === STORE, true.B, false.B)
    io.ctrl_WBSel := MuxLookup(io.opcode,0.U,Seq(
        //R-type
        //Please fill in the blanks by yourself
        //I-type
        OP     -> 1.U,
